...
export interface VisemeUnits {
  VisemeAnalyzer: new (sampleRate?: number) => VisemeAnalyzer;
  visemeWorkletMain(scope: WorkletScopeLike): void;
}

/**
 * Analyser and worklet body declared inside one function so that its `toString()` is a
 * complete module regardless of how the SDK was minified (renamed bindings, anonymous
 * classes). With `scope` given (inside the worklet) the processor is registered directly.
 */
function visemeWorkletUnits(scope?: WorkletScopeLike): VisemeUnits {
   ...
   * input, gathers 20 ms frames and posts one `VisemeFrame` per frame to the main thread.
   */
  function visemeWorkletMain(scope: WorkletScopeLike): void {
  ...
  if (scope) visemeWorkletMain(scope);
  return { VisemeAnalyzer, visemeWorkletMain };
}

const units = visemeWorkletUnits();
export const VisemeAnalyzer = units.VisemeAnalyzer;
export const visemeWorkletMain = units.visemeWorkletMain;
...
export function visemeWorkletSource(): string {
  return `'use strict';\n(${visemeWorkletUnits.toString()})(globalThis);\n`;
}
