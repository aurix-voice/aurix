...
/** Stages and worklet body as they exist inside the worklet (see `voiceEffectsWorkletUnits`). */
export interface VoiceEffectsUnits {
  BiquadStage: new (kind: 'lowpass' | 'highpass', hz: number, q: number, sampleRate: number) => VoiceEffectStage;
  PitchShiftStage: new (semitones: number, sampleRate: number) => VoiceEffectStage;
  FormantShiftStage: new (semitones: number, sampleRate: number) => VoiceEffectStage;
  RingModulatorStage: new (carrierHz: number, sampleRate: number) => VoiceEffectStage;
  DistortionStage: new (drive: number) => VoiceEffectStage;
  TremoloStage: new (rateHz: number, depth: number, sampleRate: number) => VoiceEffectStage;
  StaticStage: new (level: number) => VoiceEffectStage;
  ReverbStage: new (size: number, damping: number, mix: number, sampleRate: number) => VoiceEffectStage;
  VoiceEffectChain: new (p: ResolvedVoiceEffectParams, sampleRate: number) => VoiceEffectChain;
  voiceEffectsWorkletMain(scope: WorkletScopeLike): void;
}
 ...
 */
export interface VoiceEffectChain extends VoiceEffectStage {
  readonly stages: VoiceEffectStage[];
  readonly isBypass: boolean;
}

/**
 * Everything the worklet needs, declared inside one function so that `toString()` of that
 * function is a complete, self-consistent module: minifiers rename module-level bindings and
 * turn class declarations into anonymous class expressions, which breaks serialising them
 * one by one. With `scope` given (inside the worklet) the processor is registered directly.
 */
function voiceEffectsWorkletUnits(scope?: WorkletScopeLike): VoiceEffectsUnits {
  /** Second-order IIR filter (RBJ cookbook), transposed direct form II, per-channel state. */
  class BiquadStage implements VoiceEffectStage {
   ...
   */
  class PitchShiftStage implements VoiceEffectStage {
   ...
   */
  class FormantShiftStage implements VoiceEffectStage {
    private readonly ratio: number;
  ...
  class RingModulatorStage implements VoiceEffectStage {
  ...
  class DistortionStage implements VoiceEffectStage {
  ...
  class TremoloStage implements VoiceEffectStage {
    private readonly step: number;
  ...
  class StaticStage implements VoiceEffectStage {
   ...
   */
  class ReverbStage implements VoiceEffectStage {
   ...
   * input to its output through a `VoiceEffectChain`. The chain is rebuilt on the
   * `params` message (allocation happens there, between render quanta, not per sample).
   */
  function voiceEffectsWorkletMain(scope: WorkletScopeLike): void {
  ...
  if (scope) voiceEffectsWorkletMain(scope);
  return {
    BiquadStage,
    PitchShiftStage,
    FormantShiftStage,
...
const units = voiceEffectsWorkletUnits();
export const BiquadStage = units.BiquadStage;
export const PitchShiftStage = units.PitchShiftStage;
export const FormantShiftStage = units.FormantShiftStage;
export const RingModulatorStage = units.RingModulatorStage;
export const DistortionStage = units.DistortionStage;
export const TremoloStage = units.TremoloStage;
export const StaticStage = units.StaticStage;
export const ReverbStage = units.ReverbStage;
export const VoiceEffectChain = units.VoiceEffectChain;
export const voiceEffectsWorkletMain = units.voiceEffectsWorkletMain;
...
export function voiceEffectsWorkletSource(): string {
  return `'use strict';\n(${voiceEffectsWorkletUnits.toString()})(globalThis);\n`;
}
