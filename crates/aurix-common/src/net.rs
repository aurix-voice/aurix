...
pub fn client_ip(peer: SocketAddr, forwarded_for: Option<&str>, real_ip: Option<&str>, trusted_proxies: &[ipnetwork::IpNetwork]) -> IpAddr {
    let peer_ip = peer.ip();
    ...
    if let Some(xff) = forwarded_for {
        let hops: Vec<IpAddr> = xff.split(',').filter_map(|s| s.trim().parse::<IpAddr>().ok()).collect();
...
mod tests {
    ...
    fn untrusted_peer_ignores_forwarded_headers() {
        ...
        let ip = client_ip(peer, Some("10.0.0.1"), Some("10.0.0.2"), &parse_trusted_proxies(&["10.0.0.0/8".into()]));
    ...
    fn trusted_proxy_uses_rightmost_untrusted_hop() {
        ...
        let ip = client_ip(peer, Some("198.51.100.7, 203.0.113.9, 10.0.0.2"), None, &proxies);
