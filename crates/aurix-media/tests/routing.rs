...
impl Client {
    ...
    async fn bind(&mut self, sfu: SocketAddr) {
        ...
        self.sock.send_to(&pkt.encode_authenticated(&self.session.media_key), sfu).await.unwrap();
        let mut buf = [0u8; 256];
        let (n, _) = tokio::time::timeout(Duration::from_secs(2), self.sock.recv_from(&mut buf)).await.expect("bind ack").unwrap();
        let ack = AurixPacket::decode(&buf[..n]).unwrap();
        assert_eq!(ack.header.packet_type, PacketType::SessionBindAck);
        assert!(ack.verify_auth(&self.session.media_key), "ack must be authenticated");
    }

    async fn send_audio(&mut self, sfu: SocketAddr, channel: &ChannelId, payload: &[u8]) {
        let seq = self.next_seq();
        let pkt = AurixPacket::audio(seq, seq * 960, self.session.ssrc, channel_id_hash(channel), Bytes::copy_from_slice(payload));
        self.sock.send_to(&pkt.encode_authenticated(&self.session.media_key), sfu).await.unwrap();
    }
...
async fn start_sfu() -> (SfuNode, SocketAddr) {
    let mut sfu = SfuNode::new(MediaNodeId::new(), Region::EuWest, SfuOptions { max_participants: 3, ..SfuOptions::default() });
...
async fn bound_sessions_exchange_audio_and_spoofing_is_rejected() {
    ...
    let s_a = sfu.create_session(SessionId::new(), UserId::new(), app, "a".into()).unwrap();
    let s_b = sfu.create_session(SessionId::new(), UserId::new(), app, "b".into()).unwrap();
    sfu.join_channel(&s_a.session_id, channel, ChannelConfig::default(), ChannelRole::Speaker).unwrap();
    sfu.join_channel(&s_b.session_id, channel, ChannelConfig::default(), ChannelRole::Speaker).unwrap();
    ...
    a.bind(addr).await;
    b.bind(addr).await;
    let bound = tokio::time::timeout(Duration::from_secs(1), events.recv()).await.unwrap().unwrap();
    ...
    while let Ok(Ok(ev)) = tokio::time::timeout(Duration::from_millis(200), events.recv()).await {
        if let MediaEvent::SpeakingChanged { user_id, speaking: true, .. } = ev {
            if user_id == s_a.user_id { saw_speaking = true; }
        }
    }
    assert!(saw_speaking);
    ...
    let spoof = AurixPacket::audio(99, 0, s_a.ssrc, channel_id_hash(&channel), Bytes::from_static(b"evil"));
    attacker.send_to(&spoof.encode(), addr).await.unwrap();
    attacker.send_to(&spoof.encode_authenticated(b"guess"), addr).await.unwrap();
    assert!(b.recv().await.is_none(), "spoofed audio must not be forwarded");
    assert_eq!(s_a.get_remote_addr().unwrap(), a.sock.local_addr().unwrap(), "attacker must not steal the binding");

    // Spoof 2: B (bound) tries to send with A's SSRC -> SSRC/session mismatch.
    let pkt = AurixPacket::audio(50, 0, s_a.ssrc, channel_id_hash(&channel), Bytes::from_static(b"imposter"));
    b.sock.send_to(&pkt.encode_authenticated(&s_b.media_key), addr).await.unwrap();
    assert!(a.recv().await.is_none());

    // Unauthenticated packet from the bound address is rejected when auth is required.
    let seq = a.next_seq();
    let pkt = AurixPacket::audio(seq, 0, s_a.ssrc, channel_id_hash(&channel), Bytes::from_static(b"plain"));
    a.sock.send_to(&pkt.encode(), addr).await.unwrap();
    assert!(b.recv().await.is_none());

    // Replay of an already-seen sequence is rejected.
    let replay = AurixPacket::audio(2, 0, s_a.ssrc, channel_id_hash(&channel), Bytes::from_static(b"replay"));
    a.sock.send_to(&replay.encode_authenticated(&s_a.media_key), addr).await.unwrap();
    assert!(b.recv().await.is_none());
...
async fn capacity_is_enforced_atomically_and_bind_replay_rejected() {
    ...
    for _ in 0..3 {
        sessions.push(sfu.create_session(SessionId::new(), UserId::new(), app, "x".into()).unwrap());
    }
    assert!(sfu.create_session(SessionId::new(), UserId::new(), app, "overflow".into()).is_err());
    ...
    let now = chrono::Utc::now().timestamp_millis();
    let bind = AurixPacket::session_bind(&s.session_id, s.ssrc, now, 1).encode_authenticated(&s.media_key);
    ...
    let stale = AurixPacket::session_bind(&s.session_id, s.ssrc, now - SESSION_BIND_MAX_SKEW_MS - 1000, 2)
...
async fn cross_app_sessions_cannot_join_channel() {
    ...
    let channel = ChannelId::new();
    let s1 = sfu.create_session(SessionId::new(), UserId::new(), AppId::new(), "a".into()).unwrap();
    let s2 = sfu.create_session(SessionId::new(), UserId::new(), AppId::new(), "b".into()).unwrap();
    sfu.join_channel(&s1.session_id, channel, ChannelConfig::default(), ChannelRole::Speaker).unwrap();
    assert!(sfu.join_channel(&s2.session_id, channel, ChannelConfig::default(), ChannelRole::Speaker).is_err());
}
