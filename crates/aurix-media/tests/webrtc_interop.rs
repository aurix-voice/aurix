...
impl BrowserClient {
    async fn new() -> Self {
        ...
        Self { rtc, sock, local, mid, connected: false, received: Vec::new(), rtp_time: 0, pending: Some((offer.to_sdp_string(), pending)) }
    }
}
...
impl BrowserClient {
    ...
    async fn run_for(&mut self, dur: Duration) {
        ...
        while Instant::now() < end {
            ...
            let wait = timeout.saturating_duration_since(Instant::now()).min(Duration::from_millis(20));
            match tokio::time::timeout(wait, self.sock.recv_from(&mut buf)).await {
                Ok(Ok((n, src))) => {
                    let recv = Receive { proto: Protocol::Udp, source: src, destination: self.local, contents: (&buf[..n]).try_into().unwrap() };
                    self.rtc.handle_input(Input::Receive(Instant::now(), recv)).unwrap();
                }
                _ => {
...
fn tone() -> Vec<i16> {
    (0..FRAME_SAMPLES)
        .map(|i| ((i as f32 * 440.0 * std::f32::consts::TAU / SAMPLE_RATE as f32).sin() * 0.4 * i16::MAX as f32) as i16)
        .collect()
}
...
async fn browser_and_aurx_client_hear_each_other() {
    ...
    // Native client
    let native = sfu.create_session(SessionId::new(), UserId::new(), app, "native".into()).unwrap();
    sfu.join_channel(&native.session_id, channel, ChannelConfig::default(), ChannelRole::Speaker).unwrap();
    let native_sock = UdpSocket::bind("127.0.0.1:0").await.unwrap();
    let bind = AurixPacket::session_bind(&native.session_id, native.ssrc, chrono::Utc::now().timestamp_millis(), 1);
    native_sock.send_to(&bind.encode_authenticated(&native.media_key), sfu_addr).await.unwrap();
    let mut buf = [0u8; 256];
    tokio::time::timeout(Duration::from_secs(2), native_sock.recv_from(&mut buf)).await.unwrap().unwrap();

    // Browser client
    let browser_session = sfu.create_session(SessionId::new(), UserId::new(), app, "browser".into()).unwrap();
    sfu.join_channel(&browser_session.session_id, channel, ChannelConfig::default(), ChannelRole::Speaker).unwrap();
    let mut browser = BrowserClient::new().await;
    let offer = browser.offer();
    let answer = sfu.attach_webrtc(&browser_session.session_id, &offer).unwrap();
    ...
    assert_eq!(browser_session.get_remote_addr(), Some(browser.local), "SFU learned the ICE-verified peer address");
    ...
    while let Ok(Ok((n, _))) = tokio::time::timeout(Duration::from_millis(300), native_sock.recv_from(&mut nbuf)).await {
    ...
    assert!(got_from_browser >= 3, "native client got {got_from_browser} frames from browser");

    // Native -> browser: send Opus frames; the browser receives a mixed Opus downlink stream.
    for seq in 1..=10u32 {
        let pkt = AurixPacket::audio(seq, seq * 960, native.ssrc, channel_id_hash(&channel), Bytes::from(frame.clone()));
        native_sock.send_to(&pkt.encode_authenticated(&native.media_key), sfu_addr).await.unwrap();
        browser.run_for(Duration::from_millis(20)).await;
    }
    browser.run_for(Duration::from_millis(300)).await;
    assert!(browser.received.len() >= 3, "browser received {} mixed frames", browser.received.len());
    ...
    assert!(pcm.iter().any(|s| s.abs() > 200), "downlink carries real (non-silent) audio");

    // Disconnect cleanup
    sfu.destroy_session(&browser_session.session_id).unwrap();
    assert!(!sfu.webrtc_manager().unwrap().has_session(&browser_session.session_id));
