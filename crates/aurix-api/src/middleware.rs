...
impl ApiKeyContext {
    fn from_row(row: &ApiKeyRow) -> Self {
        Self { key_id: row.id, app_id: AppId::from_uuid(row.app_id), permissions: Arc::new(row.permissions.clone()) }
    }
    ...
    pub fn has(&self, permission: &str) -> bool {
        match &*self.permissions {
            serde_json::Value::Array(arr) => arr.iter().any(|p| p.as_str() == Some("*") || p.as_str() == Some(permission)),
            serde_json::Value::Object(obj) => obj.get("all").and_then(|v| v.as_bool()).unwrap_or(false),
            _ => false,
        }
    }
    ...
    pub fn require(&self, permission: &str) -> Result<(), ApiError> {
        if self.has(permission) {
            ...
            Err(AurixError::AuthorizationDenied(format!("API key lacks permission '{permission}'")).into())
        }
    }
}
...
fn resolve_client_ip(state: &AppState, request: &Request) -> IpAddr {
    let peer = request.extensions().get::<ConnectInfo<SocketAddr>>().map(|c| c.0).unwrap_or_else(|| SocketAddr::from(([0, 0, 0, 0], 0)));
...
pub async fn client_ip_middleware(State(state): State<AppState>, mut request: Request, next: Next) -> Response {
...
pub async fn auth_middleware(State(state): State<AppState>, mut request: Request, next: Next) -> Result<Response, ApiError> {
    let token = bearer_token(request.headers()).ok_or_else(|| AurixError::AuthenticationFailed("Missing bearer token".into()))?;
...
pub async fn api_key_middleware(State(state): State<AppState>, mut request: Request, next: Next) -> Result<Response, ApiError> {
    let api_key = request
        .headers()
        .get("x-api-key")
...
pub async fn admin_auth_middleware(State(state): State<AppState>, mut request: Request, next: Next) -> Result<Response, ApiError> {
    let token = bearer_token(request.headers()).ok_or_else(|| AurixError::AuthenticationFailed("Missing bearer token".into()))?;
...
pub async fn rate_limit_middleware(State(state): State<AppState>, request: Request, next: Next) -> Result<Response, ApiError> {
    ...
    let ip = request.extensions().get::<ClientIp>().map(|c| c.0).unwrap_or_else(|| resolve_client_ip(&state, &request));
    let key = format!("api:{ip}");
    let allowed = if let Some(ref redis) = state.control.redis {
        redis
            .check_rate_limit(&key, state.control.config.rate_limiting.requests_per_second, 1)
            .await
...
pub fn normalize_path(path: &str) -> String {
    ...
    let Some(root) = segments.next() else { return "/".into() };
...
mod tests {
    ...
    fn path_normalization_bounds_cardinality() {
        assert_eq!(normalize_path("/v1/channels/0190f5a6-5c1e-7b8e-9f1c-0f5a6c1e7b8e/participants"), "/v1/channels/:id/participants");
    ...
    fn api_key_permissions() {
        let ctx = ApiKeyContext { key_id: Uuid::nil(), app_id: AppId::from_uuid(Uuid::nil()), permissions: Arc::new(serde_json::json!(["channels:read"])) };
        assert!(ctx.has("channels:read"));
        assert!(!ctx.has("channels:write"));
        let legacy = ApiKeyContext { permissions: Arc::new(serde_json::json!({"all": true})), ..ctx.clone() };
        assert!(legacy.has("anything"));
        let star = ApiKeyContext { permissions: Arc::new(serde_json::json!(["*"])), ..ctx };
