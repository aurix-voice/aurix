...
| 10001 | UDP | SFU↔SFU cascade relay (`media.port + 1`) — node-to-node only; with `region_tree` only the regional hubs need cross-region reachability, in-region nodes need each other and their hub |
