...
| `aurix_noise_suppression_sessions`, `aurix_noise_suppression_frames_total{path,outcome}` | sessions the node is denoising (`media.noise_suppression.max_sessions` bounds it) and the frames it saw per path (`opus`/`pcmu`): `ok` cleaned, `repeated` a copy of the previous frame for another channel (cleaned once), `passthrough` too short / DTX, `skipped` a channel requirement the node could not honour (disabled or full), `error` decode/encode failure (forwarded as it came) |
