...
| Chat | `GET/POST /v1/channels/{id}/messages`, `GET/POST /v1/users/{id}/messages`, `GET /v1/users/{id}/chat-devices`, `DELETE /v1/users/{id}/chat-devices/{device_id}` ([per-device delivery cursors](../features/chat.md#exactly-once-per-device)) | API key |
