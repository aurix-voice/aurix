...
A device id given the same way (`X-Aurix-Device`, `device.<id>`, `?device=`) stays with the session
across resume and cross-node takeover.
...
| `SetAudioCodec { codec }` | `AudioCodecChanged { codec }` | native AURX only; `codec` = `opus` (default) / `pcmu` / `pcma` — G.711 fallback transcoded by the node in plaintext channels, relayed sealed in E2EE ones, see [codecs](../features/channels.md#codecs-opus-and-the-pcmu-fallback); `CODEC_NOT_AVAILABLE` when `media.pcmu_fallback` is off or the session is WebRTC |
...
| `ChatMarkRead { channel_id \| user_id, message_id }`, `ChatReadMarkers { channel_id \| user_id }` | `ChatReadMarker { marker }`, `ChatReadMarkersResult { …, markers, unread_count }`, `ChatInboxSynced { delivered, truncated, per_device }` after `SessionInitAck` | stored chat only; see [read markers](../features/chat.md#read-markers-and-unread-counts) and [offline delivery](../features/chat.md#offline-delivery-of-directed-messages) |
| `ChatAck { message_id }` (connection identified with `X-Aurix-Device: <id>` / sub-protocol `device.<id>` / `?device=`) | — (`NOT_FOUND` for a message not addressed to you) | per-device exactly-once queue of directed messages; see [exactly once per device](../features/chat.md#exactly-once-per-device) |
| `ChatEdit { message_id, text, metadata?, client_ref? }`, `ChatDelete { message_id, client_ref? }` | `ChatMessageUpdated { message }` to everyone who sees the message (`edited_at`, or a tombstone with `deleted_at` / `deleted_by`); the actor's copy carries `client_ref` | stored chat only; author within `chat.edit_window_secs`, channel moderators may delete ([edits and deletions](../features/chat.md#edits-and-deletions)) |
| `ChatReact { message_id, reaction, add }` | `ChatReactionChanged { message_id, channel_id?, message_from_user_id, message_to_user_id?, user_id, reaction, added, count, timestamp }` to everyone who sees the message; nothing on a no-op | stored chat only; `message.reactions[]` carries `{reaction, count, user_ids}` tallies ([reactions](../features/chat.md#reactions)) |
| `ChatSearch { channel_id? \| user_id?, query, from_user_id?, before?, limit?, client_ref? }` | `ChatSearchResult { channel_id?, user_id?, query, messages, next_before?, client_ref? }` | stored chat only; PostgreSQL web-search syntax, newest first, deleted messages excluded ([search](../features/chat.md#search)) |
