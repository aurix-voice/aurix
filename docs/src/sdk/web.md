...
| Server-side noise suppression | `setServerNoiseSuppression(enabled)`, `serverNoiseSuppression`, `serverNoiseSuppressionChanged(enabled)`, `sessionInfo.noiseSuppression` | the node denoises this session's uplink instead of / in addition to the browser's `getUserMedia` suppressor; rejected (`NOISE_SUPPRESSION_UNAVAILABLE`) when the node has `media.noise_suppression` off or full; replayed after a reconnect; never for E2EE channels or stereo — see [server-side noise suppression](../features/channels.md#server-side-noise-suppression) |
...
| Chat | `sendMessage()`, `sendDirectMessage()`, `setTyping()`, `chatMessage`, `participantTyping`; stored chat: `history(scope, {before, after, limit})`, `markRead(scope, messageId)`, `readMarkers(scope)`, `chatReadMarker`, `chatInboxSynced` (`perDevice`), `message.offline`, `deviceId` option for the [exactly-once per-device queue](../features/chat.md#exactly-once-per-device), `editMessage()`, `deleteMessage()`, `react()`, `search(scope, query)`, `chatMessageUpdated`, `chatReactionChanged` | see [Text chat](../features/chat.md) |
...
| Voice effects | `AurixClient.supportsVoiceEffects()`, `setVoiceEffects('robot' \| 'monster' \| 'radio' \| 'helium' \| 'ghost' \| VoiceEffectParams \| undefined)`, `voiceEffects`, `voiceEffectPreset()`, `sanitizeVoiceEffects()` | `AudioWorklet` on the microphone path only (after input gain, before the encoder); injected audio and the downlink are untouched; see [voice effects](../features/speech.md#voice-effects) |
