...
| Chat | `SendMessageAsync`, `SendDirectMessageAsync`, `SetTypingAsync`, `OnChatMessage` (`Offline`), `OnParticipantTyping`; stored chat: `HistoryAsync` / `DirectHistoryAsync` (cursor pages), `MarkReadAsync` / `MarkDirectReadAsync`, `ReadMarkersAsync` / `DirectReadMarkersAsync`, `OnChatReadMarker`, `OnChatInboxSynced`, `EditMessageAsync` / `DeleteMessageAsync`, `ReactAsync`, `SearchAsync` / `SearchDirectAsync`, `OnChatMessageUpdated`, `OnChatReactionChanged` |
